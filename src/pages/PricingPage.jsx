...
export function PricingPage() {
  ...
  const [selectedPlans, setSelectedPlans] = useState(
   ...
   // NEW ADDITION: State for confirmation modal
 ...
 useEffect(() => {
  ...
  const handlePlanSelection = (planName, value) => {
    ...
    console.log("User Job Title:", jobTitle); // Debugging line
    return allowedTitles.includes(jobTitle?.toLowerCase());
  };

  // NEW ADDITION: Function to initiate subscription
    ...
    // NEW ADDITION: Function to confirm subscription and call backend
      const confirmSubscription = async () => {
        setIsLoading(true);
        try {
          const planInterval = selectedPlans[selectedPlan.name];
          const amountKey = planInterval === "sixMonth" ? "sixMonthRate" : planInterval === "twelveMonth" ? "twelveMonthRate" : "monthlyRate";
          const startDate = isTrial ? new Date(Date.now() + 28 * 24 * 60 * 60 * 1000).toISOString().split("T")[0] : null;
    
          // Assume plan_code is fetched or mapped (replace with actual API call if needed)
          const planCode = `${activeTab.toUpperCase()}_${selectedPlan.name.toUpperCase()}_${planInterval.toUpperCase()}`; // e.g., BUYER_BRONZE_SIXMONTH

          const csrfToken = getCookie("csrftoken");
                console.log("cookie", csrfToken);
    
          const response = await authAxios.post("/subscriptions/subscribe/", {
            params: {
          ...
          // Open Paystack payment in new window
   ...
   return (
     <div className="min-h-screen bg-gray-100">
       ...
       <div className="container mx-auto px-4 py-12">
         {/* NO CHANGES: Header and tabs remain unchanged */}
         ...
         {/* UPDATED: Modified card layout to handle subscription buttons */}
           ...
           {plans.map((plan) => (
             ...
             >
               ...
               <ul className="space-y-2 mb-6">
                 {/* NO CHANGES: Plan features list remains unchanged */}
               ...
               {/* UPDATED: Replaced Link components with buttons for authenticated users */}
         ...
         {/* NO CHANGES: Compare Plans Button and Comparison Table remain unchanged */}
         <div className="text-center mb-12">
           <button
         ...
         {/* NEW ADDITION: Confirmation Modal */}
             ...
             {selectedPlan && (
               <div className="space-y-4">
                 ...
                 <p>
                   ...
                   {selectedPlans[selectedPlan.name] === "sixMonth"
                     ? "6 Months"
                     : selectedPlans[selectedPlan.name] === "twelveMonth"
                     ? "12 Months"
                     : "Monthly"}
                 </p>
                 <p>
