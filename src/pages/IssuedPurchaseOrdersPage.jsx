...
const IssuedPurchaseOrdersPage = () => {
  ...
  useEffect(() => {
    ...
    //   toast.error("Only lead buyers can view issued purchase orders.");
    //   navigate("/dashboard");
    //   return;
    // }
    const fetchPurchaseOrders = async () => {
      try {
        ...
        console.log("IssuedPurchaseOrdersPage: Purchase orders fetched:", response.data);
        setPurchaseOrders(response.data || []);
      } catch (error) {
        toast.error("Failed to load purchase orders.");
        console.error("IssuedPurchaseOrdersPage: Fetch error:", error.response?.data || error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-gray-50 py-10">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Issued Purchase Orders</h1>
          <button
