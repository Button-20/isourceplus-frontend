...
const PaymentOrdersPage = () => {
  ...
  useEffect(() => {
    ...
    //   toast.error("Only sales or logistics managers can view payment orders.");
    //   navigate("/dashboard");
    //   return;
    // }
  ...
  const handlePageChange = async (url, page) => {
    ...
    try {
      ...
      setPaymentOrders(response.data.results || []);
      setPagination({
        count: response.data.count || response.data.length,
        next: response.data.next,
        previous: response.data.previous,
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600 text-lg">Loading Payment Orders...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 py-10">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Issued Payment Orders</h1>
          <button
        ...
        <div className="bg-white shadow-lg rounded-lg p-6">
          {paymentOrders.length > 0 ? (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {paymentOrders.map((order) => (
                      <tr
                        key={order.ref_num}
                        onClick={() => navigate(`/dashboard/payment-orders/${order.ref_num}`)}
            ...
            <p className="text-gray-600">No payment orders available.</p>
          )}
        </div>
      </div>
    </div>
